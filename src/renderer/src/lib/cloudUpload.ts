...
export async function publishGallery(
  ...
  const { data: gallery, error: galleryError } = await supabase
    .from('galleries').insert(galleryPayload).select('id, slug').single()
  ...
  // Clean, short share URL — pixflow-ai.com/<business>/<gallery-slug> (the slug
  // is auto-generated by a DB trigger). Falls back to the legacy /gallery/<id>
  // form only if a slug somehow isn't present. Old UUID links keep working.
  const gallerySlug = (gallery as { slug?: string | null }).slug
  const publicUrl = gallerySlug
    ? `${GALLERY_BASE}/${slug}/${gallerySlug}`
    : `${GALLERY_BASE}/${slug}/gallery/${galleryId}`
