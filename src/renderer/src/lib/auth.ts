...
// load — the main process intercepts the navigation via webRequest and
// extracts the OAuth payload from query / fragment before any HTTP request
// fires. This URL must be added to Supabase → Authentication → URL
// Configuration → Redirect URLs. It is intentionally fixed (not derived from
// window.location.origin) so it stays in sync with what's registered in
// Supabase regardless of which port electron-vite serves the renderer on.
