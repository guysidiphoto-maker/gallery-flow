...
//
// Phase 6 step 4 (server-side update_gallery_settings RPC + Zod) uses this
// same shape on both sides — the RPC validates the JSON patch against these
// rules before merging into the row, and the dashboard's writers go through
// the RPC.
//
// Hand-rolled validator (no `zod` dep) — the schema is small enough that the
// extra bundle weight isn't worth it, and the RPC re-validates server-side
// anyway. Returns a typed result instead of throwing so call sites can
...
// forever because there was no allowlist. The RPC uses the same set;
...
const RULES: Record<string, FieldRule> = {
  ...
  // Nav / watermark — used by the design panel; keep permissive but typed.
  navStyle:             maxLength('סגנון ניווט', 32),
  watermarkEnabled:     boolean('סימן מים מופעל'),
  watermarkText:        maxLength('טקסט סימן מים', 120),
  watermarkPosition:    maxLength('מיקום סימן מים', 32),
}
