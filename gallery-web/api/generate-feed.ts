...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (jsonStart === -1 || jsonEnd === -1)
    return res.status(502).json({ ok: false, error: 'llm_returned_no_json', tail: llmText.slice(-300), length: llmText.length })
  ...
  try {
    ...
    return res.status(502).json({
      ...
      tail: llmText.slice(-400), length: llmText.length,
    })
  }
