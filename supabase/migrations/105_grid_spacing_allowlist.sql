-- ─────────────────────────────────────────────────────────────────────────────
-- 105_grid_spacing_allowlist.sql — Fix the Design → Grid "spacing" control.
--
-- The Design tab has a "מרווח בין תמונות" (grid spacing) control that writes
-- delivery_settings.gridSpacing = 'regular' | 'large'. But `gridSpacing` was
-- NEVER in the server allowlist (migration 086's _validate_delivery_settings_
-- patch), so update_gallery_settings rejected it as unknown_key and the value
-- silently never persisted — the control did nothing. The public viewer already
-- reads gridSpacing, so adding it here makes the control actually work.
--
-- Changes vs 086 (this reproduces the 086 body VERBATIM and adds only these):
--   • v_oneof_keys gains  'gridSpacing' → ['regular','large']
--   • imageSpacing gains  'wide'  (the viewer maps gridSpacing='large' → a wide
--     gap, kept in sync with the client schema so a legacy 'wide' write is also
--     accepted).
--
-- NON-DESTRUCTIVE: no columns, RLS, grants, or existing keys change. Existing
-- galleries validate exactly as before. Idempotent (CREATE OR REPLACE),
-- reversible via 105_grid_spacing_allowlist_rollback.sql.
-- ─────────────────────────────────────────────────────────────────────────────
...
DECLARE
  ...
  v_oneof_keys CONSTANT JSONB := jsonb_build_object(
    ...
    -- NEW (105): Design → Grid spacing control.
...
--   jsonb_build_object('gridSpacing','large')
-- );  -- expect NULL (valid)
-- SELECT public._validate_delivery_settings_patch(
--   jsonb_build_object('gridSpacing','bogus')
-- );  -- expect [{"key":"gridSpacing","error":"not_in_allowed_values"}]
