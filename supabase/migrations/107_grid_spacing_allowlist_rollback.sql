-- Rollback for 107_grid_spacing_allowlist.sql — restores the pre-107 (086)
-- validator: drops 'gridSpacing' from the allowlist and 'wide' from imageSpacing.
-- After this, a gridSpacing write rejects as unknown_key again. This is the
-- immediately-prior (Production-faithful baseline) definition of
-- _validate_delivery_settings_patch, reproduced executably so the down-path
-- actually restores the function (this file was previously a comment-only no-op).
